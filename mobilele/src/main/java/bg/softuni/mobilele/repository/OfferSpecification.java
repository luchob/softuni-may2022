...
public class OfferSpecification implements Specification<OfferEntity> {
  ...
  public Predicate toPredicate(Root<OfferEntity> root,
    ...
    if (searchOfferDTO.getModel() != null) {
      p.getExpressions().
          add(cb.and(cb.equal(root.join("model").get("name"), searchOfferDTO.getModel())));
    }

    if (searchOfferDTO.getMinPrice() != null) {
      p.getExpressions().
          add(cb.greaterThanOrEqualTo(root.get("price"), searchOfferDTO.getMinPrice()));
    }

    if (searchOfferDTO.getMaxPrice() != null) {
      p.getExpressions().
          add(cb.lessThanOrEqualTo(root.get("price"), searchOfferDTO.getMaxPrice()));
    }
